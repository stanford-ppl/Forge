...
trait Kmeans extends DHDLApplication {
  ...
  def kmeans(points_in: Rep[Array[Flt]], numPoints: Rep[SInt], numCents: Rep[SInt], numDims: Rep[SInt]) = {
    ...
    val P0 = param(32);   domainOf(P0) = (1,96,4)     // Dimensions loaded in parallel
    val P1 = param(8);    domainOf(P1) = (1,12,2)     // Sets of points calculated in parallel
    val P2 = param(1);    domainOf(P2) = (1,192,8)   // Dimensions accumulated in parallel (outer)
    val P3 = param(6);    domainOf(P3) = (1,16,2)     // Points calculated in parallel
    ...
    Accel {
      ...
      Fold(N by BN par P1, P2)(newCents, 0.as[Flt]){i =>
        ...
        Fold(BN par P3, P2)(centTile, 0.as[Flt]){pt =>
          ...
          Pipe(K by 1, DP1 by 1){(ct,d) =>
            val elem = mux(d == D.value, 1.as[Flt], pts(pt, d))
      ...
      Pipe(K by 1 par PX){ct => centCount(ct) = newCents(ct,D+1) }
