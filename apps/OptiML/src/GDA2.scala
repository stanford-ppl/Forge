...
object GDA2Compiler extends OptiMLApplicationCompiler with GDA2
object GDA2Interpreter extends OptiMLApplicationInterpreter with GDA2

// unrolled version of GDA
trait GDA2 extends OptiMLApplication {
  def printUsage = {
    println("Usage: GDA <input data file> <output label data file>")
    exit(-1)
  }

  def infix_outer(v1: Rep[DenseVector[Double]], v2: Rep[DenseVector[Double]]): Rep[DenseMatrix[Double]] = {
    (0::v1.length, 0::v2.length) { (i,j) =>
      v1(i) * v2(j)
    }
  }

  def infix_trans(v: Rep[DenseVector[Double]]): Rep[DenseVector[Double]] = {
    v.map(i => i)
  }
  ...
  def main() = {
    ...
    var sigma = DenseMatrix.ones(mu0.length,mu0.length)
    var i = 0
    while (i < m) {
      val s = 
        if (y(i) == false){
