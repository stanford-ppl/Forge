...
object NNCompiler extends OptiMLApplicationCompiler with NeuralNetworkClassifier 
object NNInterpreter extends OptiMLApplicationInterpreter with NeuralNetworkClassifier 
...
trait NeuralNetworkClassifier extends OptiMLApplication {

	private val MIN_WEIGHTS_INIT = -0.1
	private val MAX_WEIGHTS_INIT = 0.1
	...
	//Not used
	private def squareError(value1: Rep[Double], value2: Rep[Double]) = {
		sqrt(pow(value1 - value2, 2))
	}

	def train(dataFilename: Rep[String], labelsFilename: Rep[String], generalizationSetFilename: Rep[String], generalizationSetLabelsFilename: Rep[String], testSetFilename: Rep[String], outputFilename: Rep[String], numIterations: Rep[Int], learningRate: Rep[Double], numHidNeurons: Rep[Int]) = {
		...
		def checkForFileFormatErrors() = {
			if(labels.numRows != trainingSet.numRows) {printError("Training set and labels files do not have matching number of rows. Training set rows: " + 
											trainingSet.numRows + ". Number of label rows: " + labels.numRows + ". Each training example needs a matching label!")}
			if(testSet.numRows != testSetLabels.numRows) {printError("Generalization set and generalization set labels do not have the same number of rows. Set rows:" + 
											testSet.numRows + ". Label rows: " + testSetLabels.numRows + ". Each generalization example needs a matching label!")}
		...
		println("Number of ouputs: " + numOutputNeurons)
		...
		def updateWeights() = {
			inputToHiddenWeights = (0::numInputNeurons, 0::(numHiddenNeurons - 1)) { (i, j) =>
				inputToHiddenWeights(i, j) + inputToHiddenDeltas(i, j)
			}

			//println("Input to hidden weights: ")
			//inputToHiddenWeights.pprint

			hiddenToOutputWeights = (0::numHiddenNeurons, 0::numOutputNeurons) { (i, j) =>
				hiddenToOutputWeights(i, j) + hiddenToOutputDeltas(i, j)
			}
	...
	def print_usage = {
		println("Usage: NNCompiler <training set filename> <labels filename> <generalization set filename> <generalization set labels> <test set filename> <test set output labels filename> <optional numIterations> <optional learningRate> <optional numHiddenNeurons>")
		exit(-1)
	}

	def main() = {
		if (args.length < 4 || args.length > 9) print_usage
		val trainArgs =
			if (args.length == 6) DenseVector(args(0), args(1), args(2), args(3), args(4), args(5), NUM_ITERATIONS_INIT, LEARNING_RATE_INIT, HIDDEN_NEURON_INIT)
			else if (args.length == 7) DenseVector(args(0), args(1), args(2), args(3), args(4), args(5), args(6), LEARNING_RATE_INIT, HIDDEN_NEURON_INIT)
			else if (args.length == 8) DenseVector(args(0), args(1), args(2), args(3), args(4), args(5), args(6), args(7), HIDDEN_NEURON_INIT)
			else DenseVector(args(0), args(1), args(2), args(3), args(4), args(5), args(6), args(7), args(8))

		train(trainArgs(0), trainArgs(1), trainArgs(2), trainArgs(3), trainArgs(4), trainArgs(5), trainArgs(6).toInt, trainArgs(7).toDouble, trainArgs(8).toInt)
	}
}
