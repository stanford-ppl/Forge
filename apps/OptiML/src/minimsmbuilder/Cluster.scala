...
object MiniMSMCompiler extends OptiMLApplicationCompiler with Clarans {
  val mTheo: Manifest[Theo] = manifest[Theo]
}
object MiniMSMInterpreter extends OptiMLApplicationInterpreter with Clarans {
  val mTheo: Manifest[Theo] = manifest[Theo]
}
...
trait Clarans extends OptiMLApplication with TheoData with DirectSolver {
   ...
   */
  def rmsd_centered(realLen: Rep[Int], xCentered: Rep[DenseVector[XYZ]], yCentered: Rep[DenseVector[XYZ]], gx: Rep[Float], gy: Rep[Float]) = {
    ...
    // this is the key matrix multiply that must be vectorized!
    val M = mX * mY
  ...
  def rmsd(x: Rep[DenseVector[XYZ]], y: Rep[DenseVector[XYZ]]): Rep[Float] = {
    val xSum = sum(x)
    val ySum = sum(y)
    val xCentered = x - pack((xSum.x / x.length, xSum.y / x.length, xSum.z / x.length))
    val yCentered = y - pack((ySum.x / y.length, ySum.y / y.length, ySum.z / y.length))
  ...
  def kcenters(theo: Rep[Theo], k: Rep[Int]) = {
    ...
    pack((IndexVector(centerIndices), assignments.unsafeImmutable, distanceList.unsafeImmutable))
  }
  ...
  def clarans(ptraj: Rep[Theo],
              ...
              medoidsMethod: Rep[String] = "kcenters"): Rep[IndexVector] = {

    val numFrames = len(ptraj)

    val (initialMedoids, initialAssignments, initialDistance): (Rep[IndexVector],Rep[DenseVector[Int]],Rep[DenseVector[Float]]) = unpack {
      ...
      while (j < maxNeighbors) {
        ...
        val trialMedoid =
          if (!localSwap) {
            ...
            randomElem(assignments.find(_ == medoids(medoidI))) // TODO: appears broken (returns 0-length vector on some iteration)
          }
  ...
  def main() = {
    ...
    //     //val frame = Vector[XYZ](10,true).map(e => ((randomGaussian.toFloat,randomGaussian.toFloat,randomGaussian.toFloat)))
    //     //val traj = Matrix[XYZ](50000,10).mapRows(e => Vector[XYZ](10,true).map(e=>((randomGaussian.toFloat,randomGaussian.toFloat,randomGaussian.toFloat))))
    //     val frame = readVectorAndParse[XYZ](pathToTheoData + "_benchmark_frame.dat", v => ((v(0).toFloat,v(1).toFloat,v(2).toFloat)), ",")
