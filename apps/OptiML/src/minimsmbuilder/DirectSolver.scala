...
trait DirectSolver extends OptiMLApplication { 
  ...
  def quarticEquationSolveExact(d0: Rep[Double], d1: Rep[Double], d2: Rep[Double], d3: Rep[Double], d4: Rep[Double]): Rep[QuarticResult] = {                                                          
    ...
    val (r1,r2,nr12) = unpack {
      if (dd2 >= 0.0) {
        val D = sqrt(dd2)
        pack((-0.25*a3 + 0.5*R - 0.5*D, -0.25*a3 + 0.5*R + 0.5*D, unit(2)))
      }
      else {
        val t = -0.25*a3 + 0.5*R
        pack((t, t, unit(0)))
    ...
    val (r3,r4,nr34) = unpack {
      if (e2 >= 0.0) {
        val E = sqrt(e2)
        pack((-0.25*a3 - 0.5*R - 0.5*E, -0.25*a3 - 0.5*R + 0.5*E, unit(2)))
      }
      else {
        val t = -0.25*a3 - 0.5*R
        pack((t, t, unit(0)))
