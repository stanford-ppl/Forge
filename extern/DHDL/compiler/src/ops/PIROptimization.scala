...
trait PIROptimization extends Traversal with PIRCommon {
  ...
  def removeRouteThruStages(cu: ComputeUnit) = cu match {
    case cu: BasicComputeUnit if cu.tpe == InnerPipe && cu.parent.isDefined =>
      ...
      cu.stages = removeStages(stages, bypassStages)
  ...
  def swapGlobal(orig: GlobalMem, swap: GlobalMem) = cus.foreach { cu =>
    allMapStages(tu).foreach{stage => swapGlobal_Stage(stage, orig, swap)}
    tu.srams.foreach{sram => swapGlobal_SRAM(sram, orig, swap)}
  ...
  def swapGlobal_Reg(reg: LocalRef, orig: GlobalMem, swap: GlobalMem) = reg match {
