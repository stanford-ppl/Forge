...
trait SumOpsExp extends SumOps with BaseFatExp with EffectExp {
  ...
  override def mirror[A:Manifest](e: Def[A], f: Transformer)(implicit ctx: SourceContext): Exp[A] = {
    e match {
      ...
      case Reflect(e@Sum(st,en,b,init), u, es) => reflectMirrored(Reflect(new { override val original = Some(f,e) } with Sum(f(st),f(en),f(b),f(init))(mtype(e.m), atype(e.a), cstype(e.cs), e.sc), mapOver(f,u), f(es)))(mtype(manifest[A]), pos)
      case Reflect(e@SumIf(st,en,c,b), u, es) => reflectMirrored(Reflect(new { override val original = Some(f,e) } with SumIf(f(st),f(en),f(c),f(b))(mtype(e.mR),atype(e.a),mtype(e.mA),cstype(e.cs),e.sc), mapOver(f,u), f(es)))(mtype(manifest[A]), pos)
