...
trait LAPACKHelperOps {
  ...
  def ipiv_to_P(numRows: Rep[Int], ipiv: Rep[DenseVector[Int]]): Rep[DenseMatrix[Int]]/*TR = {
  ...
  def postprocess_lu(a: Rep[DenseMatrix[Double]], ipiv: Rep[DenseVector[Int]])(implicit __pos: SourceContext): (Rep[DenseMatrix[Double]],Rep[DenseMatrix[Double]],Rep[DenseMatrix[Int]])/*TR = {
  ...
  def check_chol(a: Rep[DenseMatrix[Double]], tri: Rep[String]): Rep[Unit]/*TR =  {
    if (tri != "upper" && tri != "lower") fatal("chol: illegal 'tri' string")
    if (a.numRows != a.numCols) fatal("chol: A is not symmetric")
  }*/

  def postprocess_chol(a: Rep[DenseMatrix[Double]], tri: Rep[String]): Rep[DenseMatrix[Double]]/*TR =  {
    if (tri == "upper") {
      a.triu
    }
    else {
