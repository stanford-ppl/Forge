...
import templates.doc.ForgeCodeGenSphinx
...
trait ForgeApplicationRunner extends ForgeApplication with ForgeExp {
  ...
  final def main(args: Array[String]) {
    ...
    def transient(name: String, backend: BackendType, tpePars: List[Rep[TypePar]], args: List[Rep[DSLType]], retTpe: Rep[DSLType], effect: EffectType): Rep[DSLOp] = {
      Op(grp("transient"), name, directMethod, backend, tpePars, listToArgs(args), Nil, arg("__pos",MSourceContext), retTpe, effect, nohint)
    }

    val T = tpePar("T")

    val aAlloc = transient("array_raw_alloc", internalBackend, T, (MArray(T),MInt), MArray(T), mutable)
    val aLength = transient("array_length", sharedBackend, T, MArray(T), MInt, pure)
    val aApply = transient("array_apply", sharedBackend, T, (MArray(T),MInt), T, pure)
    val aUpdate = transient("array_update", internalBackend, T, (MArray(T),MInt,T), MUnit, write(0))
    parallelize (MArray) as ParallelCollection(T, aAlloc, aLength, aApply, aUpdate)

    val abAlloc = transient("array_buffer_raw_alloc", internalBackend, T, (MArrayBuffer(T),MInt), MArrayBuffer(T), mutable)
    val abLength = transient("array_buffer_length", internalBackend, T, MArrayBuffer(T), MInt, pure)
    val abApply = transient("array_buffer_apply", internalBackend, T, (MArrayBuffer(T),MInt), T, pure)
    val abUpdate = transient("array_buffer_update", internalBackend, T, (MArrayBuffer(T),MInt,T), MUnit, write(0))
    val abSetLength = transient("array_buffer_set_length", internalBackend, T, (MArrayBuffer(T),MInt), MUnit, write(0))
    val abAppendable = transient("array_buffer_appendable", internalBackend, T, MArrayBuffer(T), MBoolean, pure)
    val abAppend = transient("array_buffer_dcappend", internalBackend, T, (MArrayBuffer(T),MInt,T), MUnit, write(0))
    val abCopy = transient("array_buffer_copy", internalBackend, T, (MArrayBuffer(T),MInt,MArrayBuffer(T),MInt,MInt), MUnit, write(2))
    ...
    val sphinxCodegen = new ForgeCodeGenSphinx {
      val IR: ForgeApplicationRunner.this.type = ForgeApplicationRunner.this
    }
    ...
    if (Config.genSphinx) codeGenerators :+= sphinxCodegen
