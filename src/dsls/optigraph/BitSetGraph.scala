...
trait BitSetGraphOps{
  ...
  def importBitSetGraphOps() {
    ...
    data(BitSetGraph,("_numNodes",MInt),("_heavyNodes",SHashMap(MInt,MInt)),("_nbrHash",MArray(MHashMap(MInt,MInt))),("_externalIDs",MArray(MInt)),("_nodes",MArray(MInt)),("_edges",MArray(MInt))) 
    static(BitSetGraph)("apply", Nil, (MethodSignature(List(("count",MInt),("shash",SHashMap(MInt,MInt)),("nbrHash",MArray(MHashMap(MInt,MInt))),("exID",MArray(MInt)),("outNodes",MArray(MInt)),("outEdges",MArray(MInt))), BitSetGraph))) implements allocates(BitSetGraph,${$count},${$shash},${$nbrHash},${$exID},${$outNodes},${outEdges})
