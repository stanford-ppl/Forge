...
trait NodeDataOps {
  ...
  def importNodeDataOps() {
    ...
    val V = tpePar("V")
      ...
      infix ("append") (T :: MUnit, effect = write(0)) implements single ${nd_append($self,$self.length, $1)}
			//method to get an array of data to outside world
			infix ("getRawDataArray") (Nil :: MArray(T)) implements single ${array_buffer_result(nd_raw_data($self))}
