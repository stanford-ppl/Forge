...
trait DenseMatrixOps {
  ...
  def importDenseMatrixOps() {
    ...
    // --- Data fields
    data(DenseMatrix, ("_data", MArray2D(T)))
    compiler (DenseMatrix) ("densematrix_raw_data", T, DenseMatrix(T) :: MArray2D(T)) implements getter(0, "_data")
    compiler (DenseMatrix) ("densematrix_set_raw_data", T, (DenseMatrix(T), MArray2D(T)) :: MUnit) implements setter(0, "_data", ${$1})

    // --- Matrix Constructors
    compiler (DenseMatrix) ("densematrix_fromarray2d", T, MArray2D(T) :: DenseMatrix(T)) implements allocates(DenseMatrix, ${$0})
    ...
    static (DenseMatrix) ("apply", T, (MInt, MInt) :: DenseMatrix(T), effect = mutable) implements composite ${ 
      densematrix_fromarray2d(Array2D[T]($0, $1))
    }
    static (DenseMatrix) ("flattenRows", T, DenseVector(DenseVector(T)) :: DenseMatrix(T)) implements composite ${
      (0::$0.length, 0::$0(0).length) {(i,j) => $0(i).apply(j) }
    }
    static (DenseMatrix) ("flattenCols", T, DenseVector(DenseVector(T)) :: DenseMatrix(T)) implements composite ${
    ...
    static (DenseMatrix) ("apply", T, DenseVector(DenseVector(T)) :: DenseMatrix(T)) implements composite ${
      (0::$0.length, 0::$0(0).length) {(i,j) => $0(i).apply(j) }
    }
    ...
    static (DenseMatrix) ("apply", T, varArgs(DenseVector(T)) :: DenseMatrix(T)) implements composite ${
      ...
      val out = DenseMatrix[T](numRows, numCols)
    ...
    direct (DenseMatrix) ("block", T, varArgs(DenseVector(DenseMatrix(T))) :: DenseMatrix(T)) implements composite ${
      var totalRows = 0
      var totalCols = 0
      ...
      // write blocks
      val out = DenseMatrix[T](totalRows, totalCols)
      var row = 0
      var col = 0
      for (i <- 0 until numargs) {
        val subMatRow = array_apply(mats, i)
    ...
    direct (DenseMatrix) ("diag", T, DenseVector(T) :: DenseMatrix(T), TArith(T)) implements composite ${ 
      val arith = implicitly[Arith[T]]
    ...
    direct (DenseMatrix) ("identity", Nil, MInt :: DenseMatrix(T)) implements redirect ${ identity($0, $0) }

    direct (DenseMatrix) ("diag", T, DenseMatrix(T) :: DenseVector(T)) implements redirect ${ $0.diag }
    direct (DenseMatrix) ("triu", T, DenseMatrix(T) :: DenseMatrix(T), TArith(T)) implements redirect ${ $0.triu }
    direct (DenseMatrix) ("tril", T, DenseMatrix(T) :: DenseMatrix(T), TArith(T)) implements redirect ${ $0.tril }
    ...
    DenseMatrixOps {
      ...
      //infix ("apply") ((MInt, MInt) :: T) implements composite ${ densematrix_raw_data($self).apply($1, $2) }
      ...
      // Moved to TensorCommonOps
      /*infix ("slice") ((IndexVector, IndexVector) :: DenseMatrix(T)) implements composite ${
      ...
      infix ("slice") ((MInt, MInt, MInt, MInt) :: DenseMatrix(T)) implements redirect ${ $self.slice($1::$2,$3::$4) }
      
      infix ("getRow") (MInt :: DenseVector(T)) implements composite ${ 
        densevector_fromarray1d(densematrix_raw_data($0).sliceRow($1), true)
      }
      infix ("getCol") (MInt :: DenseVector(T)) implements composite ${ 
        densevector_fromarray1d(densematrix_raw_data($0).sliceCol($1), false)
      }
      infix ("apply") (MInt :: DenseVector(T)) implements composite ${ $self.getRow($1) }
      infix ("sliceRows") (IndexVector :: DenseMatrix(T)) implements composite ${ $self.slice($1, *) }
      infix ("sliceCols") (IndexVector :: DenseMatrix(T)) implements composite ${ $self.slice(*, $1) }
      infix ("sliceRows") ((("start",MInt),("end",MInt)) :: DenseMatrix(T)) implements composite ${ $self.slice($start::$end, *) }
      infix ("sliceCols") ((("start",MInt),("end",MInt)) :: DenseMatrix(T)) implements composite ${ $self.slice(*, $start::$end) }
      ...
      // Moved to TensorCommonOps
      /*infix ("dice") ((IndexVector, IndexVector) :: DenseMatrix(T)) implements composite ${
      ...
      infix ("diceRows") (IndexVector :: DenseMatrix(T)) implements composite ${ $self.dice($1, *) }
      infix ("diceCols") (IndexVector :: DenseMatrix(T)) implements composite ${ $self.dice(*, $1) }

      // --- Ambiguous methods
      // Moved to TensorCommonOps
      /*infix ("apply") ((IndexVector, IndexVector) :: DenseMatrix(T)) implements composite ${ 
      ...
      // e.g. m(1, 0::3)
      infix ("apply") ((MInt, IndexVector) :: DenseVector(T)) implements composite ${
        if (isWild($2)) $self.getRow($1)
        else if (isRange($2)) $self.getRow($1).slice($2)
        else $self.getRow($1).dice($2)
      }
      infix ("apply") ((IndexVector, MInt) :: DenseVector(T)) implements composite ${
      ...
      // Should still be able to figure out windowed update vs. broadcast from here
			infix ("update") ((IndexVector, IndexVector, T) :: MUnit, effect = write(0)) implements composite ${
      ...
      infix ("update") ((IndexVector, IndexVector, DenseMatrix(T)) :: MUnit, effect = write(0)) implements composite ${
      ...
      // multi row/column updates
			infix ("update") ((IndexVector, DenseMatrix(T)) :: MUnit, effect = write(0)) implements composite ${ $self($1, *) = $2 }
      infix ("updateRows") ((IndexVector, DenseMatrix(T)) :: MUnit, effect = write(0)) implements composite ${ $self($1, *) = $2 }
      infix ("updateCols") ((IndexVector, DenseMatrix(T)) :: MUnit, effect = write(0)) implements composite ${ $self(*, $1) = $2 }
     	
      // full row/column updates
      infix ("updateRow") ((MInt, DenseVector(T)) :: MUnit, effect = write(0)) implements composite ${ 
        (0::$2.length) foreach {j => $self($1, j) = $2(j) }
      }
      infix ("updateCol") ((MInt, DenseVector(T)) :: MUnit, effect = write(0)) implements composite ${ 
      ...
      infix ("update") ((MInt, IndexVector, DenseVector(T)) :: MUnit, effect = write(0)) implements composite ${
        val cInds = if (isWild($2)) $self.colIndices else $2
        cInds.forIndices{j => $self($1, cInds(j)) = $3(j) }
      }
      infix ("update") ((IndexVector, MInt, DenseVector(T)) :: MUnit, effect = write(0)) implements composite ${
      ...
      infix ("<<") (DenseVector(T) :: DenseMatrix(T)) implements composite ${
        ...
        val out = DenseMatrix[T]($self.numRows + 1, $self.numCols)
      ...
      infix ("<<") (DenseMatrix(T) :: DenseMatrix(T)) implements composite ${
        ...
        val out = DenseMatrix[T]($self.numRows + $1.numRows, $self.numCols)
        out($self.rowIndices, *) = $self
      ...
      infix ("<<|") (DenseVector(T) :: DenseMatrix(T)) implements composite ${
        ...
        val out = DenseMatrix[T]($self.numRows, $self.numCols + 1)
      ...
      infix ("<<|") (DenseMatrix(T) :: DenseMatrix(T)) implements composite ${
        ...
        val out = DenseMatrix[T]($self.numRows, $self.numCols + $1.numCols)
        out(*, $self.colIndices) = $self
      ...
      infix ("<<=") (DenseMatrix(T) :: MUnit, effect = write(0)) implements composite ${ $self.insertRows($self.numRows, $1) }
      infix ("<<|=") (DenseVector(T) :: MUnit, effect = write(0)) implements composite ${ $self.insertCol($self.numCols, $1) }
      infix ("<<|=") (DenseMatrix(T) :: MUnit, effect = write(0)) implements composite ${ $self.insertCols($self.numCols, $1) }
      infix ("insertAllRows") ((MInt, DenseMatrix(T)) :: MUnit, effect = write(0)) implements composite ${ $self.insertRows($1, $2) }
      infix ("insertAllCols") ((MInt, DenseMatrix(T)) :: MUnit, effect = write(0)) implements composite ${ $self.insertCols($1, $2) }

      infix ("insertRow") ((MInt, DenseVector(T)) :: MUnit, effect = write(0)) implements composite ${
        densematrix_raw_data($self).insertRow($1, densevector_raw_data($2))
      }
      infix ("insertCol") ((MInt, DenseVector(T)) :: MUnit, effect = write(0)) implements composite ${
        densematrix_raw_data($self).insertCol($1, densevector_raw_data($2))
      }
      infix ("insertRows") ((MInt, DenseMatrix(T)) :: MUnit, effect = write(0)) implements composite ${
        densematrix_raw_data($self).insertRows($1, densevector_raw_data($2))
      }
      infix ("insertCols") ((MInt, DenseMatrix(T)) :: MUnit, effect = write(0)) implements composite ${
      ...
      infix ("clear") (Nil :: MUnit, effect = write(0)) implements composite ${
        ...
        densematrix_set_raw_data($self, Array2D[T](0,0).unsafeImmutable)
      }
      ...
      infix ("*:*") (DenseMatrix(T) :: DenseMatrix(T), TArith(T)) implements composite ${
        $0.zip($1){(a,b) => implicitly[Arith[T]] * (a,b) }
      }
      ...
      infix ("minIndexX1") (Nil :: Tuple2(MInt, MInt), TOrder(T)) implements composite ${
        ...
        densematrix_raw_data($self).zip(inds2D){(a,b) => pack((a,b)) }.reduce{ (a,b) =>
          if (implicitly[Order[T]].lt(a._1, b._1)) a._2 else b._2 
        }
      }
      infix ("maxIndexX1") (Nil :: Tuple2(MInt, MInt), TOrder(T)) implements composite ${
        val inds2D = Array2D.fromFunction($self.numRows, $self.numCols){(i,j) => pack((i,j)) }
        densematrix_raw_data($self).zip(inds2D){(a,b) => pack((a,b)) }.reduce{ (a,b) =>
          if (implicitly[Order[T]].gt(a._1, b._1)) a._2 else b._2 
        }
      }
      ...
      infix ("minIndexX2") (Nil :: Tuple2(MInt, MInt), TOrder(T)) implements composite ${
        val flatIndex = $self.reshape($self.size).minIndex
        unflatten(flatIndex, pack(($self.numRows, $self.numCols)))
      }
      infix ("maxIndexX2") (Nil :: Tuple2(MInt, MInt), TOrder(T)) implements composite ${
        val flatIndex = $self.reshape($self.size).maxIndex
        unflatten(flatIndex, pack(($self.numRows, $self.numCols)))
      }

      infix ("minIndex") (Nil :: Tuple2(MInt, MInt), TOrder(T)) implements composite ${
        var min = $self(0, 0)
        var minRow = 0
        var minCol = 0
        val i = 0
        ...
        for (i <- 0 until $self.numRows) {
          for (j <- 0 until $self.numCols) {
            if (implicitly[Order[T]].lt($self(i, j), min)) {
              min = $self(i, j)
              minRow = i
              minCol = j
            }
      ...
      infix ("maxIndex") (Nil :: Tuple2(MInt, MInt), TOrder(T)) implements composite ${
        var max = $self(0, 0)
        var maxRow = 0
        var maxCol = 0
        val i = 0
        ...
        for (i <- 0 until $self.numRows) {
          for (j <- 0 until $self.numCols) {
            if (implicitly[Order[T]].gt($self(i, j), max)) {
              max = $self(i, j)
              maxRow = i
              maxCol = j
            }
      ...
      // --- Permuting
      infix ("t") (Nil :: DenseMatrix(T)) implements composite ${ densematrix_fromarray2d(densematrix_raw_data($self).t) }
      infix ("vt") (Nil :: DenseMatrix(T)) implements composite ${ densematrix_fromarray2d(densematrix_raw_data($self).vt) }

      // --- Reshaping
      infix ("flattenToVector") (MBoolean :: DenseVector(T)) implements composite ${
        densevector_fromarray1d(densematrix_raw_data($self).reshape($self.size), $1)
      }
      infix ("flattenToVector") (Nil :: DenseVector(T)) implements composite ${ $self.flattenToVector(true) }
      ...
      infix ("minRows") (Nil :: DenseVector(T), TOrder(T)) implements composite ${ $self.mapRowsToVector{row => min(row) }}
      infix ("minCols") (Nil :: DenseVector(T), TOrder(T)) implements composite ${ $self.mapColsToVector{col => min(col) }}
      infix ("maxRows") (Nil :: DenseVector(T), TOrder(T)) implements composite ${ $self.mapRowsToVector{row => max(row) }}
      infix ("maxCols") (Nil :: DenseVector(T), TOrder(T)) implements composite ${ $self.mapColsToVector{col => max(col) }}

      infix ("mapRowsToVector") ((DenseVector(T) ==> R) :: DenseVector(R), addTpePars = R) implements composite ${
        $self.rowIndices.map{i => $1( $self.getRow(i) )}
      }
      infix ("mapColsToVector") ((DenseVector(T) ==> R) :: DenseVector(R), addTpePars = R) implements composite ${
        $self.colIndices.map{j => $1( $self.getCol(j) )}
      }

      infix ("findRows") ((DenseVector(T) ==> MBoolean) :: IndexVector) implements composite ${
        $self.rowIndices.filter{i => $1( $self(i, *) )}
      }
      infix ("findCols") ((DenseVector(T) ==> MBoolean) :: IndexVector) implements composite ${
        $self.colIndices.filter{j => $1( $self(*, j) )}
      }

      infix ("filterRows") ((DenseVector(T) ==> MBoolean) :: DenseMatrix(T)) implements composite ${
        $self($self.findRows($1))
      }
      infix ("filterCols") ((DenseVector(T) ==> MBoolean) :: DenseMatrix(T)) implements composite ${
        $self(*, $self.findCols($1))
      }

      // row/col foreach
      infix ("foreachRow") ((DenseVector(T) ==> MUnit) :: MUnit, effect = simple) implements composite ${
        $self.rowIndices foreach {i => $1($self(i, *)) }
      }
      infix ("foreachCol") ((DenseVector(T) ==> MUnit) :: MUnit, effect = simple) implements composite ${
      ...
      infix ("mapRows") ((DenseVector(T) ==> DenseVector(R)) :: DenseMatrix(R), addTpePars = R) implements composite ${
        val data = densematrix_raw_data($self).mapRows{row => densevector_raw_data($1(densevector_fromarray1d(row, true))) }
        densematrix_fromarray2d(data)
      }
      infix ("mapCols") ((DenseVector(T) ==> DenseVector(R)) :: DenseMatrix(R), addTpePars = R) implements composite ${
      ...
      infix ("groupRowsBy") ((DenseVector(T) ==> K) :: MMap(K, DenseMatrix(T)), addTpePars = K) implements composite ${
      ...
      infix ("groupColsBy") ((DenseVector(T) ==> K) :: MMap(K, DenseMatrix(T)), addTpePars = K) implements composite ${
    ...
    compiler (DenseMatrix) ("raw_data", T, DenseMatrix(T) :: MArray2D(T)) implements composite ${ densematrix_raw_data($0) }
