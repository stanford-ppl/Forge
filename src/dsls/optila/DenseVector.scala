...
trait DenseVectorOps {
  this: OptiLADSL =>
    ...
    // Data fields
    data(DenseVector, ("_data", MArray1D(A)), ("_isRow", MBoolean))
    compiler (DenseVector) ("densevector_raw_data", A, DenseVector(A) :: MArray1D(A)) implements getter(0, "_data")
    compiler (DenseVector) ("densevector_set_raw_data", A, (DenseVector(A), MArray1D(A)) :: MUnit) implements setter(0, "_data", ${$1})
    compiler (DenseVector) ("densevector_set_is_row", A, (DenseVector(A), MBoolean) :: MUnit) implements setter(0, "_isRow", ${$1})
    compiler (DenseVector) ("isWild", A, DenseVector(A) :: MBoolean) implements composite ${ unit(false) }
    compiler (DenseVector) ("isRange", A, DenseVector(A) :: MBoolean) implements composite ${ unit(false) }

    // --- Vector Constructors
    compiler (DenseVector) ("densevector_fromarray1d", A, (MArray1D(A), MBoolean) :: DenseVector(A)) implements allocates(DenseVector, ${$0}, ${$1})
    compiler (DenseVector) ("densevector_fromfunc", A, (MInt, (MInt ==> A)) :: DenseVector(A)) implements redirect ${ densevector_fromfunc($0, $1, unit(true)) }
    compiler (DenseVector) ("densevector_fromfunc", A, (MInt, (MInt ==> A), MBoolean) :: DenseVector(A)) implements composite ${
    ...
    for (arity <- (2 until 23)) {
      // We use "Reppable" to allow heterogeneous tuples (e.g. A, Rep[A], Var[A]) to still be converted
    ...
    static (DenseVector) ("apply", A, (MInt, MBoolean) :: DenseVector(A), effect = mutable) implements composite ${ densevector_fromarray1d(Array1D[T]($0), $1) }
    static (DenseVector) ("apply", T, varArgs(A) :: DenseVector(A)) implements composite ${ densevector_fromarray1d(fmultia_fromseq($0), unit(true)) }

    // Changed from a sequential loop with updates to a flatMap on the outer vector
    static (DenseVector) ("flatten", A, DenseVector(DenseVector(A)) :: DenseVector(A)) implements composite ${
    ...
    static (DenseVector) ("fromFile", A, (("path", MString), ("schemaBldr", DenseVector(MString) ==> A)) :: DenseVector(A)) implements composite ${
      DenseVector.fromFile($path, "\"\\s+\"", $schemaBldr)
    }
    static (DenseVector) ("fromFile", A (("path", MString), ("delim", MString), ("schemaBldr", DenseVector(MString) ==> A)) :: DenseVector(A)) implements wrapper ${
    ...
    infix (DenseVector) ("toVector", (A,R), MMap(A, R) :: DenseVector(R)) implements composite ${
    ...
    DenseVectorOps {
      ...
      infix ("apply") (MInt :: A) implements composite ${ densevector_raw_data($self).apply($1) }

      // --- Continuous accesses (view)
      infix ("slice") (IndexVector :: DenseVector(A)) implements composite ${
      ...
      infix ("dice") (IndexVector :: DenseVector(A)) implements composite ${ 
      ...
      infix ("update") ((MInt, A) :: MUnit, effect = write(0)) implements composite ${
      ...
      // v(*) = x, v(0::n) = x
      infix ("update") ((IndexVector, A) :: MUnit, effect = write(0)) implements composite ${
      ...
      // v(*) = v2, v(0::n) = v2
      infix ("update") ((IndexVector, DenseVector(A)) :: MUnit, effect = write(0)) implements composite ${
        if (isWild($1)) { 				// v(*) = v2
      ...
      infix ("copyFrom") ((MInt, DenseVector(A)) :: MUnit, effect = write(0)) implements composite ${
        $self($1::($1 + $2.length)) = $2
      }

      // --- Vector joins
      infix ("<<") (A :: DenseVector(A)) implements composite ${
        val out = DenseVector[A]($self.length + 1, $self.isRow)
        out.copyFrom(0, $self)
        out($self.length) = $1
        out.unsafeImmutable
      }
      infix ("<<") (DenseVector(A) :: DenseVector(A)) implements composite ${
        val out = vector_with_order[A]($self.length + $1.length, $self.isRow)
        out.copyFrom(0, $self)
      ...
      infix ("<<=") (A :: MUnit, effect = write(0)) implements composite ${ $self.append($1) }
      infix ("<<=") (DenseVector(A) :: MUnit, effect = write(0)) implements composite ${ $self.appendAll($1) }
      ...
      infix ("appendAll") (DenseVector(A) :: MUnit, effect = write(0)) implements composite ${
        densevector_raw_data($self).appendAll(densevector_raw_data($1))
      }
      infix ("insertAll") ((MInt, DenseVector(A)) :: MUnit, effect = write(0)) implements composite ${
      ...
      infix ("clear") (Nil :: MUnit, effect = write(0)) implements composite ${
        densevector_set_raw_data($self, Array1D[A](0).unsafeImmutable)
      }
      ...
      // uses original version of .sort with TOrdering(A) restriction
      infix ("oldSort") (Nil :: DenseVector(A), TOrdering(A)) implements composite ${
        val a = densevector_raw_data($self).sort
        densevector_fromarray1d(a, $self.isRow)
      }
      infix ("sort") (Nil :: DenseVector(A), TOrder(A)) implements composite ${
        val data = densevector_raw_data($self).sortWith{(a,b) => implicitly[Order[A]].compare(a,b) }
        densevector_fromarray1d(data, $self.isRow)
      }
      infix ("sortWith") (((A,A) ==> MInt) :: DenseVector(A)) implements composite ${
        val data = densevector_raw_data($self).sortWith{(a,b) => $1(a,b)}
        densevector_fromarray1d(data, $self.isRow)
      } 

      // essentially a sortWith + dice
      infix ("sortWithIndex") (Nil :: CTuple2(DenseVector(A), IndexVector), TOrder(A)) implements composite ${
        val indsRaw = Array1D.sortIndices($self.length){(a,b) => implicitly[Order[A]].compare($self(a), $self(b)) }
        val inds = indexvector_fromarray(inds, $self.isRow)
        ($self(inds), inds)
      }

      // Outputs double, not A. e.g. Vector(1, 2) is still expected to have median of 1.5, not 1
      // TODO: this doesn't work right now on anything but Doubles (in original OptiML either)
      infix ("median") (Nil :: MDouble, (TNumeric(A), TOrder(A))) implements composite ${
        val x = $self.sort
        val mid = x.length / 2
        if (x.length % 2 == 0) {
      ...
      direct ("median") (Nil :: MDouble, (TNumeric(A), TOrder(A))) implements composite ${ $0.median }


      // --- List-like operations
      // Has the advantage of early stopping, but sequential
      infix ("contains") (A :: MBoolean) implements composite ${ 
        var found = false
        var i = 0
      ...
      infix ("tailContains") (A :: MBoolean) implements composite ${
        var found = false
        var i = $self.length
        while (i > 0 && !found) {
          i -= 1
      ...
      infix ("redContains") (A :: MBoolean) implements composite ${
        $self.map{e => e == $1}.fold(unit(false)){(a,b) => a || b}
      }

      infix ("distinct") (Nil :: DenseVector(A)) implements composite ${
        val set = SHashMap[A, Boolean]()
        val out = DenseVector[A](0, $self.isRow)
        var index = 0
      ...
      direct ("__equal") (DenseVector(A) :: MBoolean) implements composite ${
      ...
      // dot product
      infix ("*:*") (DenseVector(A) :: A, TArith(A)) implements composite ${
        fassert($self.length == $1.length, "Dimension mismatch in vector dot product (" + $self.length + " != " + $1.length + ")")
        sum($self * $1)
      }

      // vector outer product
      infix ("**") (DenseVector(A) :: DenseMatrix(A), TArith(A)) implements composite ${
      ...
      infix ("*") (DenseMatrix(A) :: DenseVector(A), TArith(A)) implements composite ${ 
      ...
      infix ("prefixSum") (Nil :: DenseVector(A), TArith(A)) implements composite ${
        $self.scan(implicitly[Arith[A]].zero($self(0))){(a,b) => implicitly[Arith[A]] + (a,b) }
      }

      infix ("minIndex") (Nil :: MInt, TOrder(A)) implements composite ${
        $self.indices.reduce {(a,b) => if (implicitly[Order[A]].lt($self(a), $self(b))) a else b }
      }
      infix ("maxIndex") (Nil :: MInt, TOrder(A)) implements composite ${
        $self.indices.reduce {(a,b) => if (implicitly[Order[A]].gt($self(a), $self(b))) a else b }
      }

      // --- Permuting / Reshaping
      // 3 Different types of permute: copying, mutating, and viewing
      infix ("t") (Nil :: DenseVector(A)) implements composite ${ 
      ...
      infix ("toMat") (Nil :: DenseMatrix(A)) implements composite ${
      ...
      // --- Vector parallel ops
      infix ("filter") ((A ==> MBoolean) :: DenseVector(A)) implements composite ${ 
    ...
    compiler (DenseVector) ("raw_data", A, DenseVector(A) :: MArray1D(A)) implements composite ${ densevector_raw_data($self) }
    addMultiArrayCommonOps(DenseVector, 1, "mul", wrapper1D)
    addVectorCommonOps(DenseVector, A)
