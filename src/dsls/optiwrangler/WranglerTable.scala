...
trait WranglerTableOps {
  ...
  def importWranglerTableOps() {
    ...
    internal (IO) ("writeToFileHelper", Nil, (("path",MString), ("headers", MString), ("rows", MArray(MString)), ("numRows", MInt)) :: MUnit, effect = simple) implements codegen($cala, ${
      ...
      */
      ...
      internal ("get_num_past_ops") (("op" -> MString) :: MInt) implements composite ${
        val a = array_filter(get_col_headers($self), ((s:Rep[String]) => s.startsWith($op)))
        array_length(a)
      }

      internal ("get_cell") ((MInt, MInt) :: MString) implements composite ${
        val data = table_data($self)
      ...
      internal ("set_cell") ((MInt, MInt, MString) :: MUnit, effect = write(0)) implements single ${
        val data = table_data($self)
    	...
    	internal ("table_data") (Nil :: MArray(Tuple2(MString, MArray(MString)))) implements getter(0, "data")
    	internal ("table_num_cols") (Nil :: MInt) implements getter(0, "numCols")
      internal ("table_num_rows") (Nil :: MInt) implements getter(0, "numRows")

  		internal ("get_column_by_name") (MString :: MArray(MString)) implements composite ${
        val data = table_data($self)
        val filtered_arr = array_filter(data, ((tpl:Rep[Tup2[String, ForgeArray[String]]]) => tup2__1(tpl) == $1))
        filtered_arr(0)._2
  		}

      internal ("empty_data") (MInt :: MArray(Tuple2(MString, MArray(MString)))) implements composite ${
        array_empty[Tup2[String, ForgeArray[String]]]($1)
      }

      internal ("get_column") (MInt :: MArray(MString)) implements composite ${
        val data = table_data($self)
        tup2__2(data($1))
      }

      internal ("get_column_from_arr") ((MArray(Tuple2(MString, MArray(MString))), MInt) :: MArray(MString)) implements composite ${
        tup2__2($1($2))
      }

      internal ("get_translate_value") (("col" -> MInt, "row" -> MInt, "direction" -> MString) :: MString) implements composite ${
        val data = table_data($self)
      ...
      internal ("get_fill_value") (("col" -> MInt, "row" -> MInt, "direction" -> MString) :: MString) implements composite ${
        val data = table_data($self)
      ...
      internal ("transpose_row") (MInt :: MArray(MString)) implements single ${
        array_fromfunction(table_num_cols($self), (c => get_cell($self, c, $1)))
      }

      internal ("get_col_headers") (Nil :: MArray(MString)) implements composite ${
        val data = table_data($self)
        array_fromfunction(table_num_cols($self), (c => tup2__1(data(c))))
      }

      internal ("get_merged_col") (("colIndices" -> MArray(MInt), "delimiter" -> MString) :: MArray(MString)) implements composite ${
        array_fromfunction(table_num_rows($self), (r => concatenate_row_elems($self, r, $colIndices, $delimiter)))
      }

      internal ("concatenate_row_elems") (("rowIndex" -> MInt, "colIndices" -> MArray(MInt), "delimiter" -> MString) :: MString) implements composite ${
      ...
      internal ("test_row") (("rowIndex" -> MInt, "startsWithColIndex" -> MInt, "startsWithValue" -> MString, "containsColIndex" -> MInt, "containsValue" -> MString) :: MBoolean) implements composite ${
      ...
      internal ("is_row_empty") (("rowIndex" -> MInt) :: MBoolean) implements composite ${
        /*
        val data = table_data($self)
      ...
      internal ("expand_col_by_elem_duplication") (("col" -> MArray(MString), "numOfRowsToExcludeFromStart" -> MInt, "duplicationFactor" -> MInt) :: MArray(MString)) implements composite ${
      ...
      internal ("generate_col_by_array_duplication") (("pattern" -> MArray(MString), "duplicationFactor" -> MInt) :: MArray(MString)) implements composite ${
      ...
      internal ("get_elems") ((MArray(MString), MArray(MInt)) :: MArray(MString)) implements composite ${
        val numElemsInRes = array_length($2)
        array_fromfunction(numElemsInRes, (i => $1($2(i))))
      }

      internal ("tuple") ((MString, MArray(MString)) :: Tuple2(MString, MArray(MString))) implements composite ${
        pack(($1, $2))
      }

      internal ("get_col_index") (MString :: MInt) implements composite ${
        var index = -1
        val data = table_data($self)
      ...
      internal ("get_col_name") (MInt :: MString) implements composite ${
        val data = table_data($self)
        data($1)._1
      }

      // TODO: Combine these two functions into one generic function.
      internal ("array_contains_int") ((MArray(MInt), MInt) :: MBoolean) implements composite ${
        val a = array_filter($1, ((e:Rep[Int]) => e == $2))
        array_length(a) > 0
      }

      internal ("array_contains_str") ((MArray(MString), MString) :: MBoolean) implements composite ${
