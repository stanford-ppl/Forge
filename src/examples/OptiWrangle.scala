...
trait OptiWrangle extends ForgeApplication with ScalaOps {
  ...
  def specification() = {
    ...
    val table_setHeader_cell = op (Table) ("setHeaderCell", infix, List(), List(Table, MInt, MString), Table, single(Table, {
      stream.printLines(
        "Table("+quotedArg(0)+".length(), "+quotedArg(0)+".data(), "+quotedArg(0)+".array_add("+quotedArg(0)+".header(), "+quotedArg(1)+","+quotedArg(2)+"))"
    )}))

    // todo - add to arrayOps
    val array_add = op (Table) ("array_add", infix, List(), List(Table, AS, MInt, MString), AS, codegenerated)
    codegen (array_add) ($cala, stream.printLines(
    "val ext = "+quotedArg(1) + ".take("+quotedArg(2)+") :+" + quotedArg(3),
    "ext ++ "+quotedArg(1)+".drop("+quotedArg(2)+")"
    ))
  ...
  //map((AS, AS, Table), 0, "e => "+quotedArg(0)+".table_cut_all_helper(e, "+quotedArg(1)+")"))
    ...
    val table_extract_map = op (Table) ("extract_map", infix, List(), List(Table, MInt, MString), Table, map((AS, AS, Table), 0, "e => "+quotedArg(0)+".table_extract_helper(e, "+quotedArg(1)+", "+quotedArg(2)+")"))
    val table_extract_helper = op (Table) ("table_extract_helper", infix, List(), List(Table, AS, MInt, MString), AS, codegenerated)
    codegen (table_extract_helper) ($cala, stream.printLines(
      "val ext : String = if ("+quotedArg(1)+"("+quotedArg(2)+").indexOf("+quotedArg(3)+") >= 0) "+quotedArg(3) +" else \"\"", 
      "val temp = " + quotedArg(1) + ".take("+quotedArg(2)+") :+ ext",
      "temp ++ "+quotedArg(1)+".drop("+quotedArg(2)+")"
    ))

    val table_extract_header = op (Table) ("extract", infix, List(), List(Table, MInt, MString), Table, single(Table, {stream.printLines(
      quotedArg(0)+".extract_map("+quotedArg(1)+","+quotedArg(2)+").setHeaderCell("+quotedArg(1)+", \"extract\")"
    )}))
    ...
    //val table_fromFile = op (Table) ("apply", static, List(), List(MString, MString, MString), Table, codegenerated)    
    ...
    val table_writeToFile = op (Table) ("write", infix, List(), List(Table, MString, MString), MUnit, codegenerated, effect = simple)
