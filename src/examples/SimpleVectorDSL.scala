...
trait SimpleVectorDSL extends ForgeApplication {
  ...
  def importVectorOps() {
    ...
    static (Vector) ("fromFunction", T, CurriedMethodSignature(List(List(MInt), List(MInt ==> T)), Vector(T))) implements composite ${
      Vector(array_fromfunction($0, $1))
    }
    ...
    VectorOps {
      ...
      // misc
      infix ("makeString") (Nil :: MString) implements composite ${
        var s = ""
        var i = 0
        while (i < $self.length) {
          s = s + $self(i) + "\\n"
      ...
      infix ("pprint") (Nil :: MUnit, effect = simple) implements composite ${ println($self.makeString) }

      //enables pretty output in the REPL
      infix ("toString") (Nil :: MString) implements composite ${ $self.makeString }
