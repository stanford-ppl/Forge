...
trait SimpleVectorDSL extends ForgeApplication with ScalaOps {
  ...
  def specification() = {
    ...
    impl (vmap) (map((T,T,Vector), 0, "e => "+quotedArg(1)+"(e)"))
    ...
    impl (vfilter) (filter((T,T,Vector), 0, "e => " + quotedArg(1) + "(e)", "e => e"))
    
    // a composite op
    val vmapreduce = op (Vector) ("mapreduce", infix, List(T withBound TNumeric), List(Vector,mapper,reducer), T)
    impl (vmapreduce) (composite(Vector, { quotedArg(0)+".map("+quotedArg(1)+").reduce("+quotedArg(2)+")" }))
    ...
    impl (vset) (map ((T,MInt,Vector), 0, "y  => "+quotedArg("x") + " + " + quotedArg("z")))

    val vplus = op (Vector) ("+", infix, List(T withBound TNumeric), List(Vector,Vector), Vector)
    impl (vplus) (zip((T,T,T,Vector), (0,1), "(a,b) => a+b"))
    ...
    impl (vinsertspace) (single(MUnit, {
      val v = quotedArg(0)
      val pos = quotedArg(1)
      val len = quotedArg(2)
    ...
    impl (vensureextra) (single(MUnit, {
      val v = quotedArg(0)
