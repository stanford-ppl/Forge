...
trait ForgeCodeGenDocBase extends ForgeCodeGenBackend with BaseGenOps {
  ...
  def opsDir: String
  def fileExt: String
  def typify(x: Exp[DSLType]): String = if (IR.useReps) repifySome(x) else typifySome(x)
  ...
  lazy val OneLineExample = "\\{\\{\\{(.*)\\}\\}\\}".r            // [Unused] Creates a one-line code example
  lazy val ExampleStart   = "\\{\\{\\{".r                         // [Unused] Begins a multi-line code example
  lazy val ExampleEnd     = "\\}\\}\\}".r                         // [Unused] Ends a multi-line code example
  ...
  def isStaticObject(grp: Rep[DSLGroup]) = {
    if (!OpsGrp.contains(grp)) false
      ...
      ops.exists(op => op.style == staticMethod && op.visibility == publicMethod) && !isDataStruct(grp)
    }
  }
  ...
  def parseOpDescription(op: Rep[DSLOp], desc: String, isTypeClass: Boolean): Option[OpDescription] = {
    ...
    var enableDoc: Boolean = op.visibility == publicMethod // don't document internal methods
    ...
    if (enableDoc) Some(OpDescription(op, opDesc.mkString(" "), args.toList, examplesFinal, returns)) else None
  }
  ...
  def createGroupDoc(grp: Rep[DSLGroup], ops: List[Rep[DSLOp]]): Option[GroupDocumentation] = {
    ...
    val opDescs = ops.flatMap{op =>
      OpDoc.get(op) match {
        ...
        case _ if op.name.startsWith("forge_") || op.visibility == privateMethod => None
