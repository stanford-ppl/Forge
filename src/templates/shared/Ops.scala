...
trait BaseGenOps extends ForgeCodeGenBase {  
  ...
  def makeImplicitCtxBounds(tpeArgs: List[Rep[TypeArg]]) = {
    tpeArgs.flatMap(a => a.ctxBounds.map(b => "implicitly["+b.name+"["+quote(a)+"]]")).mkString(",")
  }  
  def makeOpImplicitCtxBounds(o: Rep[DSLOp]) = makeImplicitCtxBounds(o.tpeArgs)    
  ...
  def implicitCtxBoundsWithType(tpeArgs: List[Rep[TypeArg]], without: List[Rep[TypeArg]] = Nil) = {
    val withoutBounds = without.flatMap(a => a.ctxBounds)
    tpeArgs.flatMap(a => a.ctxBounds.diff(withoutBounds).map(b => tpe(b.name+"["+quote(a)+"]"))).distinct // a little hacky to use tpe like this
  }  
  def makeImplicitArgsWithCtxBoundsWithType(implicitArgs: List[Rep[DSLType]], tpeArgs: List[Rep[TypeArg]], without: List[Rep[TypeArg]] = Nil, asVals: Boolean = false) = {
    makeImplicitArgsWithType(implicitCtxBoundsWithType(tpeArgs, without) ++ implicitArgs, asVals)    
  }
  ...
  // method names
  def makeFullArgs(o: Rep[DSLOp], makeArgs: Rep[DSLOp] => String) = makeTpeArgs(o.tpeArgs) + makeArgs(o) + (if (needDisambiguate(o)) makeOpImplicitArgsWithOverload(o) else "")  
  ...
  def makeSimpleMethod(o: Rep[DSLOp]) = {
    "def " + o.name + makeTpeArgsWithBounds(o.tpeArgs) + makeOpArgsWithNowType(o) + makeOpImplicitArgsWithOverloadWithType(o) + " = " + makeOpMethodNameWithFutureArgs(o)
  }
  ...
  def emitOpSyntax(ops: DSLOps, stream: PrintWriter) {
    ...
    for (o <- infixOps) {
      if (noInfixList.contains(o.name)) {
        ...
        pimpStream.appendLine("    def " + o.name + makeTpeArgsWithBounds(o.tpeArgs.diff(tpe.tpeArgs)) + otherArgs
          + (makeImplicitArgsWithCtxBoundsWithType(implicitArgsWithOverload(o), o.tpeArgs, without = tpe.tpeArgs)) + " = " + makeOpMethodNameWithFutureArgs(o))
      }
      else {
