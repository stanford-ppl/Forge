...
trait InputOutputOps extends Base {
  ...
  object ForgeFileReader {
    ...
    def readLinesChunk[A:Manifest](stream: Rep[ForgeFileInputStream])(offset: Rep[Long], numBytes: Rep[Long])(f: (Rep[String], Rep[String]) => Rep[A])(implicit pos: SourceContext): Rep[ForgeArray[A]] = forge_filereader_readlines_chunk(stream, offset, numBytes, f)
  }
...
trait InputOutputCompilerOps extends InputOutputOps with OverloadHack {
  ...
  implicit class ForgeFileOutputStreamOpsCls(stream: Rep[ForgeFileOutputStream])(implicit ctx: SourceContext) {
    def writeLine(line: Rep[String])(implicit ctx: SourceContext, o: Overloaded1) = forge_fileoutputstream_writeline(stream, line)
    def close(implicit ctx: SourceContext, o: Overloaded2) = forge_fileoutputstream_close(stream)
  }
