...
trait InputOutputOps extends Base {
  ...
  object ForgeFileWriter {
    def writeLines(path: Rep[String], numLines: Rep[Int])(f: Rep[Int] => Rep[String])(implicit ctx: SourceContext) = forge_filewriter_writelines(path, numLines, f)
  }
...
trait InputOutputCompilerOps extends InputOutputOps with OverloadHack {
  ...
  object ForgeFileOutputStream {
    def apply(path: Rep[String])(implicit ctx: SourceContext): Rep[ForgeFileOutputStream] = forge_fileoutputstream_new(path)
  }
