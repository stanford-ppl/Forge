...
trait QueryableGroupByReduceTest extends ForgeTestModule with OptiQLApplication with TestRecord {
  def main() = {
    val res1 = items GroupBy(_.status) Select(g => Record (
      status = g.key,
      sumQuantity = g.values.Sum(_.quantity),
      minPrice = g.values.Min(_.price),
    ...
    val res2 = items Where(_.quantity > 0) GroupBy(_.status) Select(g => Record (
      status = g.key
      sumQuantity = g.values.Sum(_.quantity),
      maxQuantity = g.values.Max(_.quantity),
      avgPrice = g.values.Average(_.price),
      count = g.values.Count
    ))
    collect(res2.size == 2)
...
trait QueryableJoinTest extends ForgeTestModule with OptiQLApplication with TestRecord {
  def main() = {
    val res = items.Join(items2)(_.id, _.id)((a,b) => Record (
      id = a.id,
      quantity = b.quantity
    ))
